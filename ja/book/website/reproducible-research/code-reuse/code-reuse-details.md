...
このセクションには、ソフトウェアをより再利用可能にするための推奨事項のシンプルなチェックリストが含まれています。
...
操作のシーケンスをスクリプト化することの良い副作用は、これがステップのドキュメントとして機能することが多いことです。
...
スクリプト内でファイルパスを指定する代わりに、よりポータブルで一般的で再利用可能なスクリプトのコマンドライン引数として渡すことを検討してください。
