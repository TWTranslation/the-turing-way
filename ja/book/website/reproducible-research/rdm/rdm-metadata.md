...
それが理解できない場合は、利用可能なデータは何の役にも立ちません。 出典とコンテキストを提供するメタデータがなければ、データを効果的に使用することはできません。
例えば、見出しが列/行に含まれているものを記述しない場合、数字のテーブルは役に立ちません。
したがって、オープンなデータセットに一貫性のあるメタデータ、つまりデータが完全に説明されるようにデータに関する情報が含まれていることを確認する必要があります。
...
ただし、すべての規律がメタデータ標準を使用するわけではありません。
