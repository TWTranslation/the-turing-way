...
Gitを使い始める最大の課題の1つは、ファイルとディレクトリ構造とブランチの複雑さを理解することです。 限られた視覚表現のコマンドラインでは、コミット、リモート、そしてそれらがどのように相互作用するか。
上級ユーザーの場合でも、概念図から必要なGitコマンドに変換することで、望ましい結果を得ることができます。
...
このブラウザベースのインタラクティブなツールにはチュートリアルが含まれており、仮想の "リポジトリ" と Sandbox を介して Git を使用することができます。
組み込みのチャレンジを完了することで、Git コマンドに慣れることは、雑用よりもゲームのようなものです。
