...
| {ref}`Version Control<rr-vcs>`                                             | مفيدة     | Experience using git and GitHub are helpful                                      |
...
هذا لأن تقسيم عدد صحيح يطبق على عدد صحيح في Python 2, ولكن التقسيم (عادي) يطبق على جميع الأنواع، بما في ذلك عدد صحيح، في Python 3.

Therefore this simple script returns _different_ answers depending on the computational environment in which it is run.
من السهل استخدام الإصدار الخاطئ من بايثون ، ويوضح كيف يمكن لقطعة من التعليمات البرمجية صالحة تماما أن تعطي نتائج مختلفة اعتمادا على بيئتها.
