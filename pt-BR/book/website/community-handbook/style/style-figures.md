...
Não use imagens que não estão licenciadas para reutilização.
...
Em geral, certifique-se de sempre citar a imagem corretamente conforme direcionado pelos proprietários de imagem.
"Imagem da Internet" não é suficiente.
...
Legendas aparecem abaixo da figura.
Devem ser curtos e concisos e incluir uma referência à fonte de onde são retirados.
Em particular, é importante descrever a licença ao abrigo da qual a imagem é reutilizada.

(modelo-figuras-avançadas)=

> Fazendo seu primeiro pull request no GitHub.
> Ilustração do _projeto Turing Way_ por Scriberia.
> Usada sob uma licença CC-BY 4.0.
...
- Both `:` and `"` have syntactic meaning for Sphinx.
  Isso significa que é importante que não utilize esses caracteres no seu texto alternativo.
