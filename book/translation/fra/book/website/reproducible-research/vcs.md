...
Différents systèmes de contrôle de version peuvent être utilisés à travers un programme avec une interface graphique, des applications basées sur un navigateur Web ou des outils en ligne de commande.
Des outils tels que Google Drive et Dropbox proposent des plates-formes pour mettre à jour des fichiers et les partager en temps réel en collaboration.
...
Celles-ci permettent aux collaborateurs de mettre à jour des fichiers tout en stockant chaque version dans son historique de version (nous en discuterons en détail).
...
Les pratiques de versionnage proviennent principalement de la gestion des changements dans les dépôts de code.
Cependant, en réalité, vous pouvez utiliser le contrôle de version pour presque n'importe quel type de fichier sur un ordinateur.
