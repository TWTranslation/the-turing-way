...
- **Robust:** A result is robust when the _same_ dataset is subjected to _different_ analysis workflows to answer the same research question (for example one pipeline written in R and another written in Python) and a qualitatively similar or identical answer is produced.
  Sağlam sonuçlar, çalışmanın analizi gerçekleştirmek için seçilen programlama dilinin özelliklerine bağlı olmadığını göstermektedir.
- **Generalisable:** Combining replicable and robust findings allow us to form generalisable results.
  ...
  Çalışmanın araştırma sorusunun tüm farklı yönlerine ne kadar iyi uygulandığını bilmek için daha birçok adım olacaktır.
  Genelleme, sonucun belirli bir veri kümesine veya analiz boru hattının belirli bir sürümüne bağlı olmadığını anlama yolunda atılmış önemli bir adımdır.
