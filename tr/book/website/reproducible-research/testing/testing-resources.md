...
- **Regression test:** Comparing the result of a test before and after the code has been altered. Çıktı değiştiyse, programda bir yere bir sorun oluşur ve bir hata meydana gelir.
...
- **Stochastic code:** Code which, while correct, does not always output the same result. Örneğin, on rastgele sayı üreten bir program, doğru olmasına rağmen her seferinde farklı bir sonuç üretecektir.
...
- **Unit:** A small piece of code that does one simple thing. Genellikle bir veya birkaç girdiye ve genellikle tek bir çıktıya sahiptir.
