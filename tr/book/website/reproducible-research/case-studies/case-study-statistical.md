...
# İstatistiksel Yöntemler Makalesi

## Bu vaka çalışması hakkında

Bu vaka çalışmasının amacı, istatistiksel bir çalışmanın tasarlanması ve yürütülmesinde uygulanan araştırma tekrarlanabilirliğinin farklı bileşenlerini tartışmaktır.
...
Bu, istatistikte klasik bir sorundur ve araştırma verilerinin toplanmasının aşırı pahalı olduğu ve etik konuların ele alınması gereken tıbbi istatistikte özellikle önemlidir.
Makale, sistematik olarak planlama belirsizliğini örneklem boyutu türetmesine dahil etmek için yöntemleri gözden geçirir ve genişletir.

### Alıntı özeti

Makale düz metin APA formatında alıntılanabilir:

> Kunzmann, K., Grayling, M. J., Lee, K. M., Robertson, D. S., Rufibach, K., & Wason, J. (2020).
> Doğrulayıcı denemeler için örneklem büyüklüğü türetilmesine ilişkin Bayesci bakış açılarının gözden geçirilmesi
> arXiv ön baskı arXiv:2006.15715.
...
## Tekrarlanabilir araştırma için farklı yöntemler kataloğu
...
Ancak bu, makalede kullanılan tüm şekillerin ayrı ayrı oluşturulması gerektiği anlamına gelir.
...
### Tekrarlanabilir yazılım ortamı

Bu, makaleyi sıfırdan derlemek için gereken tüm kodun bağımsız bir ortamda mevcut olduğu anlamına gelse de, tekrarlanabilirliği sağlamak için henüz yeterli değildir.
LaTeX, Jupyter ve R'ı tüm kodu çalıştırmak için gereken spesifikasyonu ile kurmak, deneyimsiz kullanıcılar için hala zor olabilir.
...
Bu, ilgilenen kişilerin
deposunun etkileşimli bir sürümünü, önceden yüklenmiş tüm gerekli yazılımlarla - tam olarak doğru
sürümlerde - başlatmalarına olanak tanır!
...
Bu rozet, veri havuzunun belirli bir zamandaki durumunu gösterir (
git etiketleme özelliği aracılığıyla).
Bu, havuzun içeriğinde daha sonra
düzeltme olsa bile bağlantıların geçerli ve değişmeden kalacağı anlamına gelir!
Binder, birden çok kullanıcı arabirimini destekler.
...
### Snakemake kullanarak iş akışı yönetimi
