...
This can be helpful as it is not always clear whether a potential confounder is in fact a confounder, or a _mediator_, a _proxy confounder_, or a _competing exposure_.
...
Modelling mediators is only required if we wish to estimate the _direct_ causal effect of the exposure on the outcome, rather than the more common _total_ causal effect.
_True confounders_ are more informative than _proxy confounders_, but proxy confounders can offer some information when true confounders are impractical to measure.
