...
Le fait de disposer de données ne sert à rien si elles ne peuvent être comprises. Sans métadonnées pour fournir la provenance et le contexte, les données ne peuvent pas être utilisées efficacement.
Par exemple, une table de nombres est inutile si aucun titre ne décrit ce que contiennent les colonnes/lignes.
Par conséquent, vous devez vous assurer que les jeux de données ouverts incluent des métadonnées cohérentes, c'est-à-dire des informations sur les données afin que les données soient pleinement décrites.
...
Les normes de métadonnées sont également spécifiques à la discipline.
...
Toutes les disciplines ne peuvent cependant pas utiliser des normes de métadonnées.
...
Il y a aussi des situations où les chercheurs utilisent des normes de métadonnées plus générales, par exemple, lorsqu'ils utilisent une archive générique pour stocker leurs données, ils doivent adhérer aux normes de métadonnées de l'archive.
