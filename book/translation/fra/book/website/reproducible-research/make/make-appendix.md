...
La dernière propriété est bien sûr très pratique pour un système de construction. More
...
We'll be using **GNU Make** in this tutorial. Vérifiez que c'est ce que vous avez en tapant :
...
repository that contains a separate rule for generating the QQ-plots. Ce Makefile ressemble à ceci :
...
Au fur et à mesure que le nombre de scripts que vous voulez exécuter sur vos données augmente, ceci peut conduire à un grand nombre de règles dans le Makefile qui sont presque exactement les mêmes. We can simplify this by creating a 
...
for escaping. Pour créer toutes les cibles, nous avons besoin d'une ligne qui appelle cette recette en conserve.  Dans notre cas, nous utilisons une boucle double sur les genres et les scripts :
